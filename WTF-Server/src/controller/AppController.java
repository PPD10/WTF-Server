...
import view.RegistrationRequest;
import view.RegistrationResponse;
...
public class AppController extends Controller {
	...
	public static void main(String[] args) {
		...
		//
		// playerModel.createPlayer("Phil�monBG", "swagg",
		// "I am a MAC address");
		//
	...
	public RegistrationResponse register(RegistrationRequest registrationRequest) {
		PlayerModel playerModel = new PlayerModel();
		int[] idDeviceParam = new int[1];

		if (playerModel.createPlayer(registrationRequest.getUsername(),
				registrationRequest.getPassword(),
				registrationRequest.getMacAddress(), idDeviceParam)) {
			int idPlayer = playerModel.getDAO().getBean().getId();
			int idDevice = idDeviceParam[0];
			
			return new RegistrationResponse(idPlayer, idDevice);
		}
